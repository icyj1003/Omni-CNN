    ...
    def init(self, args, config, model):
        ...
        for name, W in model.named_parameters():
            if name in self.prune_ratios:
                ...
                if (
                    args.admm == True and args.dynamic == True
                ):  # and name=="conv1.weight": # dont run again during fine-tuning+if dynamic pruning
...
def find_layers_pruning_ratio(args, key, param):  # in np array form
    ...
    print(
        "non_zero_elements", non_zero_elements.shape, non_zero_elements[:10]
    )  # non_zero_elements torch.Size([5654, 1])  #ok
    ...
    print(
        "len(sorted_non_zero_elements)",
        type(sorted_non_zero_elements),
        sorted_non_zero_elements.shape,
        sorted_non_zero_elements[:10],
    )
    ...
    for i in range(1, len(values)):
        ...
        if (
            values[i] < values[-1] + (values[0] - values[-1]) * args.sensetivity
...
def random_pruning(args, weight, prune_ratio):
    ...
    if args.sparsity_type == "filter":
        ...
        indices = np.random.choice(
            shape2d[0], int(shape2d[0] * prune_ratio), replace=False
        )
        weight2d[indices, :] = 0
        ...
        return (
            torch.from_numpy(expand_above_threshold).cuda(),
        ...
        if name in ADMM.prune_ratios:  # ignore layers that do not have rho
            ...
            if option == None:
                _, cuda_pruned_weights = weight_pruning(
                    args, W, ADMM.prune_ratios[name]
                )  # get sparse model in cuda
            elif option == "random":
                _, cuda_pruned_weights = random_pruning(
                    args, W, ADMM.prune_ratios[name]
                )
...
def hard_prune_mask(args, ADMM, model, option=None):
    ...
    for name, W in model.named_parameters():
        if "mask" in name:
            W.data = mask_pruning(
                args.adaptive_ratio, W, args.mask[name.replace("w_mask", "weight")]
            )
    """
    mask = {}
    for (name, W) in model.named_parameters():
    ...
    for i, (name, W) in enumerate(model.named_parameters()):
        if args.admm and name in ADMM.prune_ratios:
            ...
            _, updated_Z = weight_pruning(
                args, W, ADMM.prune_ratios[name]
            )  # Z(k+1) = W(k+1)+U(k)  U(k) is zeros her
            ADMM.ADMM_Z[name] = updated_Z

        if args.adaptive_mask and args.mask and "mask" in name:
            weight = W.cpu().detach()
            ADMM.ADMM_Y[name] = mask_pruning(
                args.adaptive_ratio, weight, args.mask[name.replace("w_mask", "weight")]
            )


def z_u_update(args, task, ADMM, model, epoch, batch_idx, writer):
    # print("^^^^^^^^^^^^^^z_u_update^^^^^^^^^^^^^^^^^^^^^")
    squared_grad_dict = dict()
    if not args.admm:
        return
    if epoch != 1 and (epoch - 1) % args.admm_epochs == 0 and batch_idx == 0:
        for i, (name, W) in enumerate(model.named_parameters()):
            ...
            if args.multi_rho:  #############added
                ...
                admm_multi_rho_scheduler(
                    ADMM, name
                )  # call multi rho scheduler every admm update
            ...
            if args.verbose:
                if writer:
                    writer.add_scalar(
                        f"task{task}_layer:{name} W(k+1)-Z(k+1)",
                        torch.sqrt(torch.sum((W - ADMM.ADMM_Z[name]) ** 2)).item(),
                        epoch,
                    )
                    writer.add_scalar(
                        f"task{task}_layer:{name} Z(k+1)-Z(k)",
                        torch.sqrt(torch.sum((ADMM.ADMM_Z[name] - Z_prev) ** 2)).item(),
                        epoch,
                    )

            ADMM.ADMM_U[name] = (
                W - ADMM.ADMM_Z[name] + ADMM.ADMM_U[name]
            )  # U(k+1) = W(k+1) - Z(k+1) +U(k)


def y_k_update(args, task, ADMM, model, epoch, batch_idx, writer):
    ...
    if epoch != 1 and (epoch - 1) % args.mask_admm_epochs == 0 and batch_idx == 0:
        for i, (name, M) in enumerate(model.named_parameters()):
            ...
            ADMM.ADMM_Y[name] = mask_pruning(
                args.adaptive_ratio,
                ADMM.ADMM_Y[name],
                args.mask[name.replace("w_mask", "weight")],
            )
            ADMM.ADMM_K[name] = (
                M - ADMM.ADMM_Y[name] + ADMM.ADMM_K[name]
            )  # U(k+1) = W(k+1) - Z(k+1) +U(k)
...
def append_admm_loss(args, ADMM, model, ce_loss):
    ...
    ):  ## initialize Z (for both weights and bias)
    ...
    for i, (name, W) in enumerate(
        ...
        if "mask" in name:
            value = (
                0.5
                * ADMM.rhos_mask[name]
                * (torch.norm(W - ADMM.ADMM_Y[name] + ADMM.ADMM_K[name], p=2) ** 2)
            )
            admm_loss[name] = (
                0.5
                * ADMM.rhos_mask[name]
...
class CrossEntropyLossMaybeSmooth(nn.CrossEntropyLoss):
    ...
    def forward(self, output, target, smooth=False):
        ...
        smooth_one_hot = one_hot * (1 - self.smooth_eps) + (
            1 - one_hot
        ) * self.smooth_eps / (n_class - 1)
        log_prb = F.log_softmax(output, dim=1)
        loss = -(smooth_one_hot * log_prb).sum(dim=1).mean()
        return loss


def mixup_data(x_common, y, alpha=1.0):
    ...
    batch_size_common = x_common.size()[0]
    index = torch.randperm(batch_size_common).cuda()

    mixed_x_common = lam * x_common + (1 - lam) * x_common[index, :]
    y_a, y_b = y, y[index]
    return mixed_x_common, y_a, y_b, lam


def mixup_criterion(criterion, pred, y_a, y_b, lam, smooth):
    return lam * criterion(pred, y_a, smooth=smooth) + (1 - lam) * criterion(
        pred, y_b, smooth=smooth
    )
...
class GradualWarmupScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [
            base_lr
            * ((self.multiplier - 1.0) * self.last_epoch / self.total_iter + 1.0)
