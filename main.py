...
def train(args, pipeline, task, train_loader, test_loader):  # 1
    ...
    if args.adaptive_mask:
        set_adaptive_mask(
            model, reset=True, requires_grad=False
        )  # set weights to 1 if 'mask' in nam or assigned value and sets gradinets [fixing weighst]

    optimizer = torch.optim.Adam(
        model.parameters(), optimizer_init_lr, weight_decay=0.001
    )  # 1
    ...
    if args.lr_scheduler == "cosine":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=args.epochs * len(train_loader), eta_min=4e-08
        )
    ...
    for epoch in range(0, args.epochs):
        ...
        pipeline.train_model(
            args, model, args.mask, train_loader, criterion, optimizer, scheduler, epoch
        ...
        print(
            ...
            len(
                np.nonzero(
                    model.conv1.weight.reshape(32 * 45 * 3 * 3).cpu().detach().numpy()
                )[0]
            ),
        )
        if prec1 > best_acc:  # saving best model
            ...
            torch.save(
                model.state_dict(), save_path + "/{}{}.pt".format(args.arch, args.depth)
            )
...
def admm_prune(args, pre_mask, task, train_loader, test_loader):  # 3
    ...
    if args.optmzr == "sgd":
        optimizer = torch.optim.SGD(
            model.parameters(), optimizer_init_lr, momentum=0.9, weight_decay=1e-4
        )
    ...
    if args.lr_scheduler == "cosine":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=args.epochs_prune * len(train_loader), eta_min=4e-08
        )
    elif args.lr_scheduler == "default":
        # my learning rate scheduler for cifar, following https://github.com/kuangliu/pytorch-cifar
        epoch_milestones = [65, 100, 130, 190, 220, 250, 280]
    ...
    if args.warmup:
        scheduler = GradualWarmupScheduler(
            ...
            total_iter=args.warmup_epochs * len(train_loader),
...
def prune_train(
    args, task, pre_mask, ADMM, train_loader, criterion, optimizer, scheduler, epoch
):  # 4
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.mixup:
            ce_loss = mixup_criterion(
                criterion, output, target_a, target_b, lam, args.smooth
            )
        else:
        ...
        if args.admm:
            admm.z_u_update(
                ...
                i,
                writer,
            )  # update Z and U  # update Z and U
            ce_loss, admm_loss, mixed_loss = admm.append_admm_loss(
                args, ADMM, model, ce_loss
            )  # append admm loss
        if args.admm_mask:
            admm.y_k_update(
                args,
...
def masked_retrain(args, pre_mask, task, train_loader, test_loader):  # 5
    ...
    if args.lr_scheduler == "cosine":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=args.epochs_mask_retrain * len(train_loader), eta_min=4e-08
        )
    elif args.lr_scheduler == "default":
        # my learning rate scheduler for cifar, following https://github.com/kuangliu/pytorch-cifar
        epoch_milestones = [65, 100, 130, 190, 220, 250, 280]
    ...
    for task in range(num_tasks):
        ...
        if task != num_tasks - 1:
            ...
            if task == 0 and args.load_model_pruned:
                ...
                load_state_dict(
                    args, model, torch.load(args.load_model_pruned)
            ...
            args.mask = copy.deepcopy(
                cumulative_mask
            )  # be precise here about args.mask
        ...
        for i in range(task + 1):
            ...
            if i != num_tasks - 1:
                ...
                if args.heritage_weight or args.adaptive_mask:
                    ...
                    prec1 = pipeline.test_model(
                        args, model, test_loader, mask_reverse(args, args.mask)
                    )
        ...
        if args.heritage_weight or args.adaptive_mask:
            load_state_dict(
                ...
                masks=mask_reverse(args, args.mask),
            )
        else:  # from zero
