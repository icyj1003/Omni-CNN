...
class CVTrainValTest:
    ...
    def load_data_flash(self, batch_size):
        ...
        self.y_train = np.asarray(
            np.argmax(np.load(os.path.join(self.base_path, "train/y.npy")), axis=1)
        )
        self.x_test = np.asarray(np.load(os.path.join(self.base_path, "test/X.npy")))
        # self.y_test = np.asarray(np.load(os.path.join(self.base_path, "test/y.npy")))
        self.y_test = np.asarray(
            np.argmax(np.load(os.path.join(self.base_path, "test/y.npy")), axis=1)
        )
        ...
        if True:  ##padding data
            print(
                "90-self.x_train.shape[1]",
                self.x_train.shape[1],
                self.x_train.shape[2],
                self.x_train.shape[3],
                90 - self.x_train.shape[1],
                160 - self.x_train.shape[2],
                20 - self.x_train.shape[3],
            )
            self.x_train = np.pad(
                self.x_train,
                [
                    (0, 0),
                    (0, 45 - self.x_train.shape[1]),
                    (0, 80 - self.x_train.shape[2]),
                    (0, 20 - self.x_train.shape[3]),
                ],
                mode="constant",
                constant_values=0,
            )
            self.x_test = np.pad(
                ...
                [
                    (0, 0),
                    (0, 45 - self.x_test.shape[1]),
                    (0, 80 - self.x_test.shape[2]),
                    (0, 20 - self.x_test.shape[3]),
                ],
                mode="constant",
                constant_values=0,
            )
    ...
    def train_model(
        self, args, model, masks, train_loader, criterion, optimizer, scheduler, epoch
    ):
